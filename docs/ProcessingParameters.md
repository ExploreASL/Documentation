...
| `M0PositionInASL4D`                    | Note that this parameter should be better provided in a studyPar.json. A vector of integers that indicates the position of M0 in TimeSeries, if it is integrated by the Vendor in the DICOM export. Will move this from ASL4D.nii to M0.nii Note that the x.modules.asl.M0PositionInASL4D parameter is independent from the x.Q.M0 parameter choice. Example for Philips 3D GRASE = '[1 2]' (first control-label pair). Example for Siemens 3D GRASE = 1 first image. Example for GE 3D spiral = 2 where first image is PWI & last = M0. Empty vector should be given (= [] or = null (in JSON)) if no action is to be taken and nothing is removed. | OPTIONAL, DEFAULT = `[] (no M0 in timeseries)` |
| `DummyScanPositionInASL4D`             | Note that this parameter should be better provided in a studyPar.json. A vector of integers that indicates the position of Dummy scans in TimeSeries if they are integrated by the Vendor in the DICOM export. This allows to remove the dummy scans or noise scans that are part of the Timeseries. A new ASL4D.nii is saved with dummy scans removed and the original is backed-up. Works in a similar way as M0PositionInASL4D, both can be entered at the same time and both indicate the original position in the Timeseries independend of each other. Example for Siemens 2D EPI = `[79 80]` Skip the control-label pair used for noise measurements. Example for certain Siemens 3D GRASE = 2 Skip the first dummy control image. Empty vector should be given (= [] or = null (in JSON)) if no action is to be taken and nothing is removed. | OPTIONAL, DEFAULT = `[] (no M0 in timeseries)` |
| `RepetitionTimePreparationM0`             | Note that this parameter should be better provided in a studyPar.json. A vector of integers that provides the repetition time (or the inversion time) for the M0scans if this differs from TR for ASL. This can be provided as an integer or as a vector and is especially useful for definining the parameters of m0scan acquired as a multi-TI inversion recovery | OPTIONAL, DEFAULT = `[]` |
...
| `bHct2BloodT1`                         | Boolean, that converts hematocrit values to blood T1 to use for quantification. Options: `0` = off, `1` = use provided Hct, `2` = use provided age & sex to derive Hct and blood-T1 | OPTIONAL, DEFAULT=use hct if exist, use age&sex if exist, otherwise disabled (in this order) |
...
| `MinimalROIVolume`                            | Specifies the minimal ROI volume in mL that is considered when calculation population statistics. All ROIs smaller than this volume are evaluated as N/A. | OPTIONAL, DEFAULT=1 |
| `bWMH` |  Boolean, TRUE = including WMH in the WM mask, FALSE = excluding WMH from the WM mask by subtracting the pvWMH (on ASL resolution) from the pvWM (on ASL resolution). It also subtracts from the GM mask but this likely doesn’t affect the GM mask. With the deep WM mask, effects are only expected with large WMH volume. | OPTIONAL, DEFAULT = FALSE|
...
{"x": {
    ...
    "S":{"Atlases": ["Thalamus", "HOcort_CONN", "Total"],"TissueMasking":["GM","GM","GM"]},
    "session":{
...
| `MRAcquisitionType`                        | String specifying the readout type. Options: `'2D'` for slice-wise readout, `'3D'` for volumetric readout. | REQUIRED |
...
| `PulseSequenceType`                          | String containing the sequence used. Options: `'spiral', 'GRASE', 'EPI'`. | REQUIRED for ASL |
